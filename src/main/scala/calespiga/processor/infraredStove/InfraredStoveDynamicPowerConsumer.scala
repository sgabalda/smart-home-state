      ...
      if (state.infraredStove.lastCommandReceived.contains(SetAutomatic)) {
        infraredStoveSyncDetector.checkIfInSync(state) match
          ...
          case _ =>
            ...
            logger
              .info(
                s"As status is ${state.infraredStove.status}, dynamic power usage for infrared stove is $result"
              )
              .as(result)
      } else {
        logger
          .info(
            s"Infrared stove is not in automatic mode, ignoring dynamic power usage"
          )
          .as(Power.zero)
      }
          ...
          desiredPowerLevel <-
            infraredStoveSyncDetector.checkIfInSync(state) match {
              case SyncDetector.NotInSync(since)
                ...
                logger
                  .info(
                    s"Infrared stove is not in sync for dynamic power usage since $since, setting power to Off"
                  )
                  .as(InfraredStoveSignal.Off)
              case _ =>
                val res =
                ...
                logger
                  .info(
                    s"Desired power level for infrared stove based on available power $powerToUse is $res"
                  )
                  .as(res)
            }
