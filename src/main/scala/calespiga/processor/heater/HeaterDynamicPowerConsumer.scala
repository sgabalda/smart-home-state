      ...
      if (state.heater.lastCommandReceived.contains(SetAutomatic)) {
        heaterSyncDetector.checkIfInSync(state) match
          ...
          case _ =>
            ...
            logger
              .info(
                s"As status is ${state.heater.status}, dynamic power usage for heater is $result"
              )
              .as(result)
      } else {
        logger
          .info(
            s"Heater is not in automatic mode, ignoring dynamic power usage"
          )
          .as(Power.zero)
      }
        ...
        for {
          desiredPowerLevel <- heaterSyncDetector.checkIfInSync(state) match {
            case SyncDetector.NotInSync(since)
              ...
              logger
                .info(
                  s"Heater is not in sync for dynamic power usage since $since, setting power to Off"
                )
                .as(HeaterSignal.Off)
            case _ =>
              val result =
              ...
              logger
                .info(
                  s"Desired power level for heater based on available power $powerToUse is $result"
                )
                .as(result)
          }
          newState = state
