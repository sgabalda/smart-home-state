...
object TemperatureRelatedProcessor {
  ...
  def apply(): TemperatureRelatedProcessor = (state: State, event: TemperatureRelated) => {
    event match {
      case TemperatureRelated.BatteryTemperatureMeasured(temperature) =>
        val newState = state.modify(_.temperatures.batteriesTemperature).setTo(temperature)
        (newState, Set.empty)

      case TemperatureRelated.ElectronicsTemperatureMeasured(temperature) =>
        val newState = state.modify(_.temperatures.electronicsTemperature).setTo(temperature)
        (newState, Set.empty)

      case TemperatureRelated.ExternalTemperatureMeasured(temperature) =>
        val newState = state.modify(_.temperatures.externalTemperature).setTo(temperature)
        (newState, Set.empty)
