...
object InputTopicMapper {
  ...
  private def generateTopicMapImpl(using Quotes): Expr[List[(String, String => EventData)]] =
    import quotes.reflect.*

    def recursiveCheck(sym: quotes.reflect.Symbol): List[(Symbol, String)] = sym.declaredTypes.flatMap { inner =>
        ...
        inner.annotations.flatMap { annotation =>
          ...
          annotation match {
            case Apply(Select(New(TypeIdent("InputEventMqtt")), _), List(Literal(StringConstant(topic)))) => 
              List((inner, topic))
            case _ => 
              List.empty
          }
        ...
        case List(paramType) =>
          paramType match {
            case tpe if tpe =:= TypeRepr.of[Boolean] =>
              val convertedValueExpr = '{ (valueStr: String) => valueStr.toBoolean}
              getNewExpr(convertedValueExpr)
            
            case tpe if tpe =:= TypeRepr.of[Double] =>
              val convertedValueExpr = '{ (valueStr: String) => valueStr.toDouble }
              getNewExpr(convertedValueExpr)

            
            case tpe if tpe =:= TypeRepr.of[Float] =>
              val convertedValueExpr = '{ (valueStr: String) => valueStr.toFloat }
              getNewExpr(convertedValueExpr)
            ...
            case tpe if tpe =:= TypeRepr.of[Long] =>
              val convertedValueExpr = '{ (valueStr: String) => valueStr.toLong }
              getNewExpr(convertedValueExpr)
            ...
            case _ =>
              report.errorAndAbort(s"Unsupported parameter type: ${paramType.show}")
          }
