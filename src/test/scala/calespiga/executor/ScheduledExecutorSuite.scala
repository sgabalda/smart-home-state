...
class ScheduledExecutorSuite extends CatsEffectSuite {
  ...
  case class DirectExecutorStub(
      executeStub: Set[Action.Direct] => IO[List[ErrorManager.Error.ExecutionError]] = _ => IO.pure(List.empty),
  ...
  test("ScheduledExecutor executes delayed action after specified delay") {
    ...
    val program = for {
      ...
      scheduledExecutor <- ScheduledExecutor(directExecutor, errorManager).use(IO.pure)
  ...
  test("ScheduledExecutor cancels previous delayed action with same ID") {
    ...
    val program = for {
      ...
      assertEquals(finalExecutedActions.length, 1, "First action should have been cancelled")
    }
  ...
  test("ScheduledExecutor handles multiple delayed actions with different IDs") {
    val directAction1 = Action.SetOpenHabItemValue("item1", "value1")
  ...
  test("ScheduledExecutor fiber cleanup after execution") {
    ...
    val program = for {
      ...
      scheduledExecutorResource = ScheduledExecutor(directExecutor, errorManager)
      ...
      assertEquals(fibersDuring.size, 1, "Fiber should be present during execution")
      assertEquals(fibersAfter.size, 0, "Fiber should be cleaned up after execution")
      assertEquals(executions, 1, "Action should have been executed exactly once")
    }
  ...
  test("ScheduledExecutor executes periodic action repeatedly") {
    ...
    val program = for {
      ...
      result <- ScheduledExecutor(directExecutor, errorManager).use { scheduledExecutor =>
        for {
      ...
      assertEquals(actionsAfterFirst.length, 1, "First execution should have occurred")
      assertEquals(actionsAfterSecond.length, 2, "Second execution should have occurred")
      assertEquals(actionsAfterThird.length, 3, "Third execution should have occurred")
  ...
  test("ScheduledExecutor cancels previous periodic action with same ID") {
    ...
    val program = for {
      ...
      result <- ScheduledExecutor(directExecutor, errorManager).use { scheduledExecutor =>
        ...
        } yield (actionsAfterFirstExecution, actionsAfterSecondExecution, finalActions)
      }
      
      (actionsAfterFirstExecution, actionsAfterSecondExecution, finalActions) = result
    } yield {
      ...
      finalActions.foreach { actions =>
        assertEquals(actions, Set(directAction2), "Only second action should execute")
      }
    }
  ...
  test("ScheduledExecutor passes periodic DirectExecutor errors to ErrorManager") {
    val directAction = Action.SetOpenHabItemValue("item1", "value1")
    val periodicAction = Action.Periodic("error-periodic", directAction, 50.millis)
    val testError = ErrorManager.Error.ExecutionError(
    ...
    val program = for {
      ...
      assertEquals(errorsAfterSecond.length, 2, "Second error should be managed")
  ...
  test("ScheduledExecutor handles mixed delayed and periodic actions") {
    ...
    val program = for {
      ...
      assertEquals(actionsAt75ms.length, 2, "Delayed execution + first periodic")
      ...
      assert(allActions.contains(delayedAction), "Delayed action should be executed")
      assert(allActions.contains(periodicAction), "Periodic action should be executed")
    }
  ...
  test("ScheduledExecutor cancels delayed action with Cancel action") {
    ...
    val program = for {
      ...
      assertEquals(actionsAfter50ms, List.empty, "Action should not execute before delay")
      assertEquals(finalActions, List.empty, "Action should not execute after cancellation")
    }
  ...
  test("ScheduledExecutor cancels periodic action with Cancel action") {
    ...
    val periodicAction = Action.Periodic("cancel-periodic", directAction, 50.millis)
    ...
    val program = for {
      ...
      assertEquals(finalActions.length, 1, "No second execution should occur after cancellation")
  ...
  test("ScheduledExecutor handles Cancel for non-existent ID gracefully") {
    ...
    val program = for {
      ...
      assertEquals(result, List.empty, "No errors should occur when cancelling non-existent ID")
    }
  ...
  test("ScheduledExecutor handles multiple Cancel actions") {
    ...
    val program = for {
      ...
      assertEquals(actionsAfterFirstPeriodic.length, 1, "Only first periodic execution should occur")
      assertEquals(finalActions.length, 1, "No further executions should occur after cancellation")
