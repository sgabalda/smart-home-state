...
class EffectfulProcessorSuite extends CatsEffectSuite {
  ...
  class AddFlagProcessor(fieldValue: Double, action: Action)
    ...
    override def process(
      ...
      val newState =
        state.modify(_.temperatures.goalTemperature).setTo(fieldValue)
  ...
  test("andThen returns state and actions in order (effectful)") {
    ...
    val processor1 = new AddFlagProcessor(42.0, action1)
