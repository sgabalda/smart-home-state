...
- **Events**: Define events in the `model/` package. Events are inputs to the program and trigger state updates and actions. There are two kinds:
  ...
  - `FeedbackEventData`: internal events generated by the program itself (via `Action.SendFeedbackEvent`) to trigger further processing, e.g. `InfraredStoveManualTimeExpired`. Avoid feedback events when the changes are to be applied immediately (in that case, use a Processor Manager -see below- for the processor and apply both changes at the same time to avoid race conditions). Use it only for events to be sent in the future (after a timeout, for example), that are required to change the state.
- **Actions**: Define actions that the program should take in response to events (e.g., setting an OpenHAB item, sending an MQTT message). Actions are returned by processors and executed by the Executors. Defined in `calespiga.model.Action`.
- **Processor Actions**: In the context of a Processor, if the same set of actions are to be sent in response to multiple events, create an Actions object. Always reuse `calespiga.processor.utils.CommandActions`, just configuring the constructor parameters. See an example in `calespiga.processor.grid.Actions`.
- **ProcessorManagers**: Inside the calespiga.processor.\* packages, when the logic to manage a device is to be used by other processors, it should be encapsulated by a `Manager`. A manager in the context of a processor package, is a stateless class that applies common state changes and generates associated actions, to be executed from multiple processors for multiple devices. If a manager is present, then the processors should call the manager based on the events received for those common actions, and not do that themselves. An example can be seen in the `calespiga.processor.grid.GridConnectionManager` and the `calespiga.processor.grid.GridConnectionProcessor`, where for enabling the grid from multiple devices, a manager is made available and is then used from the main grid processor.
